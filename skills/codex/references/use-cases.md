...
即答モード(fg=フォアグラウンド軽量クエリ)と委任モード(bg=バックグラウンド長時間委任)の実績ベースの使い分けと型。

## 即答モード(軽量・read-only・即返し)

短く・読み取りのみ・数分以内で終わるものは即答モード。`codex exec --ephemeral -s read-only` で直接実行し、出力をそのまま/要約して返す。
...
## 委任モード(長時間・write 可・委任)

書き込み・multi-file 調査・実装修正・レビュー・テスト/ビルド・所要時間不明なものは委任モード。`run.sh` を `run_in_background:true` で起動し、完了通知を待つ。詳細手順は SKILL.md。
...
read-only でも「多数ファイルを横断」「長い web 調査」で数分を超えるなら委任モード。即答モードで回すと Claude の 10 分制限に被弾する。**迷ったら委任モード**。
...
- argv 直書きせずファイル経由(run.sh は `--prompt-file`、即答モードは heredoc/一時ファイル)。quoting/injection/長さ事故を防ぐ。
- 日本語で可。技術質問で英語の方が精度が出そうなら翻訳してもよい。
- under-specify が最大の失敗要因。委任モードは特に、1 段落程度の十分な文脈を与える。
