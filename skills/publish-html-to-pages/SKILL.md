...
# それを防ぐため、生成テンプレに版番号 (TPL_VER) を持たせ、既存 index がこの skill より
...
# ヘッダ表示用にエスケープ (& < > を実体参照化)。ページ見出し/タブ名は「<repo> の Pages」。
PAGE_TITLE="$REPO の Pages"
T_TITLE=$(printf '%s' "$PAGE_TITLE"    | sed 's/&/\&amp;/g; s/</\&lt;/g; s/>/\&gt;/g')
T_SLUG=$(printf '%s' "$SLUG"           | sed 's/&/\&amp;/g; s/</\&lt;/g; s/>/\&gt;/g')
T_BRANCH=$(printf '%s' "$PAGES_BRANCH" | sed 's/&/\&amp;/g; s/</\&lt;/g; s/>/\&gt;/g')
...
HEAD
  # <title> と favicon は変数を含むので、クォート heredoc の外で printf する
  # ('HEAD' クォート heredoc は変数展開しないため、中に $ICON / $T_TITLE は書けない)。
  printf '<title>%s</title>\n' "$T_TITLE"
  printf '<link rel="icon" href="%s">\n' "$ICON"
  # 版番号スタンプ。次回 publish のバージョンガードがこれを読む (machine-readable)。
...
<body>
HEAD2
  # 見出し (= <repo> の Pages) とメタ行 (repo リンク + 公開ブランチリンク)。$PAGES_BRANCH は §3 で確定済み。
  printf '<h1>%s</h1>\n' "$T_TITLE"
  printf '<p class="meta"><a href="%s">%s</a> ・ 公開ブランチ <a href="%s/tree/%s"><code>%s</code></a></p>\n' "$WEB" "$T_SLUG" "$WEB" "$T_BRANCH" "$T_BRANCH"
