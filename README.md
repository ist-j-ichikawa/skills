...
| [`client-openai-key-client`](skills/client-openai-key-client/) | クライアント提供の OpenAI API キーを、macOS Keychain + [direnv](https://direnv.net) + リポジトリスコープの `CODEX_HOME` 経由で安全に使えるよう、対象リポジトリをセットアップ/監査する skill。生の鍵は Keychain だけに置き、`.envrc` で `OPENAI_API_KEY` を注入、[OpenAI US エンドポイント](https://us.api.openai.com/v1) を使う。Codex CLI と Claude Code の `codex-plugin-cc` が同じ設定を継承する。リポジトリの `.envrc` を読んだときだけ鍵が有効。 |
